...
namespace GPBoost {
	...
	*/
	...
	/*!
	* \brief Pivoted Cholesky factorization according to Habrecht et al. (2012) for HLFPC
	* \param L_k[out] Matrix of dimension nxmax_it such that L_k ~ ((W + D^(-1))^0.5 * B * W^0.5)^-T and rank(L_k) = k
	* \param B_rm Row-major matrix B in Vecchia approximation Sigma^-1 = B^T D^-1 B
	* \param w_sqrt Vector of dimension n, where each entry is the square root of the diagonal of W
	* \param w_plus_d_inv_sqrt_inv Vector of dimension n, where each entry is the square root of the diagonal of W + D^(-1)
	* \param max_it Max rank of L_k
	* \param num_data dtaset dimension (num_data)
	...
	*		 (W + D^(-1))^-0.5 * B-T * W * B^-1 * (W + D^(-1))^-0.5 as Q_k T_k Q_k^T whereby no preconditioner is used.
	...
	* \param generator Random number generator
	* \param B_rm Row-major matrix B in Vecchia approximation Sigma^-1 = B^T D^-1 B ("=" Cholesky factor)
	* \param w Vector of dimension n, where each entry is the diagonal of W
	* \param w_plus_d_inv_sqrt_inv Vector of dimension n, where each entry is the square root of the diagonal of (W + D^(-1))^-1
