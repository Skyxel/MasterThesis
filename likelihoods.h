...
#pragma omp parallel for schedule(static)
		...
		*/
		void FindModePostRandEffCalcMLLVecchia(const double* y_data,
			...
			InitializeLocationPar(fixed_effects, location_par, &location_par_ptr);
			// Variables for High Low Frequency Pivoted Cholesky
			den_mat_t I_k_plus_L_kt_D_W_plus_I_L_k;
			// Initialize objective function (LA approx. marginal likelihood) for use as convergence criterion
			B_mode = B * mode_;
			...
			if(matrix_inversion_method_ != "iterative" || 
				(matrix_inversion_method_ == "iterative" && (
					...
					cg_preconditioner_type_ == "incomplete_cholesky_SN_A" || 
					cg_preconditioner_type_ == "incomplete_cholesky_SN_B" || 
					cg_preconditioner_type_ == "incomplete_cholesky_ABS" || 
					cg_preconditioner_type_ == "incomplete_cholesky_JM" || 
					cg_preconditioner_type_ == "incomplete_cholesky_T" ||   
					cg_preconditioner_type_ == "incomplete_cholesky_TJM"))) {
...
#pragma omp parallel for schedule(static)
						...
						else if (cg_preconditioner_type_ == "vadu" || 
							cg_preconditioner_type_ == "incomplete_cholesky" || 
							cg_preconditioner_type_ == "incomplete_cholesky_SN_A" || 
							cg_preconditioner_type_ == "incomplete_cholesky_SN_B" || 
							cg_preconditioner_type_ == "incomplete_cholesky_ABS" || 
							...
							cg_preconditioner_type_ == "incomplete_cholesky_T" ||  
							cg_preconditioner_type_ == "incomplete_cholesky_TJM" ||  
							cg_preconditioner_type_ == "hlfpc" ||  
							cg_preconditioner_type_ == "hlfpc_nystroem_last" ||  
							cg_preconditioner_type_ == "hlfpc_nystroem_random" ||  
							cg_preconditioner_type_ == "hlfpc_pivoted_cholesky" ||  
							cg_preconditioner_type_ == "hlfpc_lanczos" ||  
							cg_preconditioner_type_ == "hlfpc_rlra") {
							if (it == 0 || information_changes_during_mode_finding_) {
								if (cg_preconditioner_type_ == "vadu") {
									Log::REInfo("Using VADU");//DELETE
									D_inv_plus_W_B_rm_ = (D_inv_rm_.diagonal() + information_ll_).asDiagonal() * B_rm_;
								}
								else if(cg_preconditioner_type_ == "hlfpc" ||  
									cg_preconditioner_type_ == "hlfpc_nystroem_last" ||  
									cg_preconditioner_type_ == "hlfpc_nystroem_random" ||  
									cg_preconditioner_type_ == "hlfpc_pivoted_cholesky" ||  
									cg_preconditioner_type_ == "hlfpc_lanczos" ||  
									...
									// Variables when using High Low frequency PC factorization
									vec_t d, d_inv, w_plus_d_inv_sqrt, w_plus_d_inv_sqrt_inv, w_sqrt, e_i, v_1, v_2, L_i;

									// (W + D^(-1))^-0.5
									d_inv = D_inv_rm_.diagonal();
									w_plus_d_inv_sqrt = (information_ll_ + d_inv).cwiseSqrt();
									w_plus_d_inv_sqrt_inv = w_plus_d_inv_sqrt.cwiseInverse();

									// P_H^0.5 = B^T * (W + D^(-1))^0.5
									P_H_sqrt_rm_ =  B_rm_.transpose() * w_plus_d_inv_sqrt.asDiagonal();

									// (DW + I)
									d = d_inv.cwiseInverse();
									d_w_plus_I_ = d.cwiseProduct(information_ll_).array() + vec_t::Ones(d.size()).array();

									// P_L = (L_k * L_k^T + (DW + I)^(-1))
									// with L = (W + D^-1)^-0.5 * B^-T * W^0.5 and L_k ~ L is an n x k matrix
									int n = d.size();
									w_sqrt = information_ll_.cwiseSqrt();

									if(cg_preconditioner_type_ == "hlfpc" ||
									   cg_preconditioner_type_ == "hlfpc_nystroem_last"){
										k_ = 50;
										L_k_.resize(n, k_);

										// Find the columns of L_k
										for (int i = 0; i < k_; ++i) {
											...
											v_2 = B_rm_.transpose().triangularView<Eigen::UpLoType::UnitUpper>().solve(v_1);
											L_i = w_plus_d_inv_sqrt_inv.cwiseProduct(v_2);
											// Assign the column to L_k
											L_k_.col(i) = L_i;
										}

										if(cg_preconditioner_type_ == "hlfpc_nystroem_last"){
											// L_k = L_k * (L_k^T * L_k)^-0.5
											den_mat_t N, N_inv_sqrt;
											N = L_k_.transpose() * L_k_;
											Eigen::SelfAdjointEigenSolver<den_mat_t> es(N);
											N_inv_sqrt = es.operatorInverseSqrt();
											L_k_ = L_k_ * N_inv_sqrt;
										}
									}
									else if(cg_preconditioner_type_ == "hlfpc_nystroem_random"){
										k_ = 50;
										L_k_.resize(n, k_);
										den_mat_t C, N, N_inv_sqrt;
										C.resize(n, k_);

										// Select k random columns
										Eigen::VectorXi indices(k_);
										for (int i = 0; i < k_; ++i) {
											indices(i) = rand() % n;
										}

										// Create a matrix C with the selected columns
										for (int i = 0; i < k_; ++i) {
											// Create the unit vector
											e_i = vec_t::Zero(n);
											e_i(indices(i)) = 1.0;

											// find the related column of L_k
											v_1 = w_sqrt.cwiseProduct(e_i);
											v_2 = B_rm_.transpose().triangularView<Eigen::UpLoType::UnitUpper>().solve(v_1);
											L_i = w_plus_d_inv_sqrt_inv.cwiseProduct(v_2);
										...
										// L_k = C * N^-0.5
										// where N is the Nystroem approximation of the covariance matrix: N = C^T * C
										N = C.transpose() * C;
									...
									else if(cg_preconditioner_type_ == "hlfpc_pivoted_cholesky"){
										k_ = 50;
										L_k_.resize(n, k_);
										const double err_tol = 1e-6;

										// Find the columns of L_k using Pivoted Cholesky factorization
										PivotedCholsekyFactorizationForHLFPC(
											L_k_, B_rm_, w_sqrt, w_plus_d_inv_sqrt_inv, k_, n, err_tol
										);
									}
									else if(cg_preconditioner_type_ == "hlfpc_lanczos"){
										k_ = 50;
										L_k_.resize(n, k_);
										double err_tol = 1e-8;
										den_mat_t Q_k;
										vec_t Tdiag_k(k_);
										vec_t Tsubdiag_k(k_ - 1);

										LanczosTridiagVecchiaLaplaceNoPreconditioner(
											cg_generator_, B_rm_, information_ll_, 
											w_plus_d_inv_sqrt_inv, n, Tdiag_k, 
											Tsubdiag_k, Q_k, k_, err_tol
										);
										
										// Compute A D A^T = T_k
										// T_k
										den_mat_t T_k = den_mat_t::Zero(k_, k_);
										T_k.diagonal() = Tdiag_k;
										T_k.diagonal(-1) = Tsubdiag_k;
										T_k.diagonal(1) = Tsubdiag_k;

										// A and D
										Eigen::SelfAdjointEigenSolver<den_mat_t> es;
										es.compute(T_k);
										den_mat_t A = es.eigenvectors();
										vec_t D = es.eigenvalues();

										// D^0.5
										den_mat_t D_sqrt = D.cwiseSqrt().asDiagonal();

										// L_k_ = Q_k * A * D^0.5
										L_k_ = Q_k * A * D_sqrt;
									}
									else if(cg_preconditioner_type_ == "hlfpc_rlra"){
										// Using Randomized Low-Rank Approximation (RLRA) for HLFPC
										// k_: target rank, p: oversampling parameter, l: k_ + p
										k_ = 50;
										int p = int(k_ / 10);
										int l = k_ + p;

										// Draw a random Gaussian matrix Omega in R^{n x l}
										den_mat_t Omega = den_mat_t::NullaryExpr(n, l, [&]() { return cg_generator_(); });

										// Compute Y = U (U^T Omega), where U = (W + D^(-1))^-0.5 * B-T * W^0.5
										den_mat_t Y(n, l);
										vec_t v_1, v_2, v_3, v_4, v_5;
										for(int i = 0; i < l; ++i) {
											v_1 = w_plus_d_inv_sqrt_inv.cwiseProduct(Omega.col(i));
											v_2 = B_rm_.triangularView<Eigen::UpLoType::UnitLower>().solve(v_1);
											v_3 = information_ll_.cwiseProduct(v_2);
											v_4 = B_rm_.transpose().triangularView<Eigen::UpLoType::UnitUpper>().solve(v_3);
											Y.col(i) = w_plus_d_inv_sqrt_inv.cwiseProduct(v_4);
										}

										// QR decomposition of Y: Y = QR (orthonormalize)
										Eigen::HouseholderQR<den_mat_t> qr(Y);
										den_mat_t Q = qr.householderQ() * den_mat_t::Identity(n, l);

										// B = Q^T * U U^T * Q
										den_mat_t B(l, l);
										for(int i = 0; i < l; ++i) {
											v_1 = w_plus_d_inv_sqrt_inv.cwiseProduct(Q.col(i));
											v_2 = B_rm_.triangularView<Eigen::UpLoType::UnitLower>().solve(v_1);
											v_3 = information_ll_.cwiseProduct(v_2);
											v_4 = B_rm_.transpose().triangularView<Eigen::UpLoType::UnitUpper>().solve(v_3);
											v_5 = w_plus_d_inv_sqrt_inv.cwiseProduct(v_4);
											B.col(i) = Q.transpose() * v_5;
										}

										// Eigen-decomposition of B = Q_tilde^T * Lambda * Q_tilde
										Eigen::SelfAdjointEigenSolver<den_mat_t> eig_solver(B);
										den_mat_t Q_tilde_k = eig_solver.eigenvectors().rightCols(k_); // top-k eigvecs
										vec_t Lambda_k = eig_solver.eigenvalues().tail(k_); // top-k eigvals

										// L_k = Q * Q_tilde_k * Lambda_k^0.5
										L_k_ = Q * Q_tilde_k * Lambda_k.cwiseSqrt().asDiagonal();
									}

									// Using Woodbury identity for P_L
									I_k_plus_L_kt_D_W_plus_I_L_k.resize(k_, k_);
									I_k_plus_L_kt_D_W_plus_I_L_k.setIdentity();
									I_k_plus_L_kt_D_W_plus_I_L_k += L_k_.transpose() * d_w_plus_I_.asDiagonal() * L_k_;
									chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_.compute(I_k_plus_L_kt_D_W_plus_I_L_k);
								}
								else {
									SigmaI_plus_W = SigmaI;
									SigmaI_plus_W.diagonal().array() += information_ll_.array();
									ReverseIncompleteCholeskyFactorization(SigmaI_plus_W, B, L_SigmaI_plus_W_rm_, cg_preconditioner_type_);
								}
							}
							...
							CGVecchiaLaplaceVec(information_ll_, B_rm_, B_t_D_inv_rm_, rhs, mode_update, has_NA_or_Inf,
								...
								D_inv_plus_W_B_rm_, L_SigmaI_plus_W_rm_, P_H_sqrt_rm_, L_k_, d_w_plus_I_,
								chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_, false);
						}
						else {
			...
			if (!has_NA_or_Inf) {//calculate determinant
				...
				if (matrix_inversion_method_ == "iterative") {
					...
					if (calc_mll) {//calculate determinant term for approx_marginal_ll
						...
						if (!saved_rand_vec_trace_) {
							...
							if (cg_preconditioner_type_ == "pivoted_cholesky") {
								...
								SigmaI_plus_W_inv_Z_.resize(dim_mode_, num_rand_vec_trace_);
									 ...
									 cg_preconditioner_type_ == "hlfpc_rlra") {
								rand_vec_trace_I_.resize(dim_mode_, num_rand_vec_trace_);
								rand_vec_trace_I2_.resize(k_, num_rand_vec_trace_);
								GenRandVecNormal(cg_generator_, rand_vec_trace_I2_);
								SigmaI_plus_W_inv_Z_.resize(dim_mode_, num_rand_vec_trace_);
							}
							else {
						...
						CalcLogDetStoch(dim_mode_, cg_max_num_it_tridiag, I_k_plus_Sigma_L_kt_W_Sigma_L_k, 
							I_k_plus_L_kt_D_W_plus_I_L_k, SigmaI, SigmaI_plus_W, B, has_NA_or_Inf, 
							log_det_Sigma_W_plus_I, re_comps_cross_cov_cluster_i, re_comps_ip_cluster_i);
						if (has_NA_or_Inf) {
						...
						for (int i = 0; i < nsim_var_pred_; ++i) {
							...
							if (cg_preconditioner_type_ == "pivoted_cholesky") {
								...
								CGVecchiaLaplaceVec(information_ll_, B_rm_, B_t_D_inv_rm_, rand_vec_pred_SigmaI_plus_W, rand_vec_pred_SigmaI_plus_W_inv, has_NA_or_Inf,
									...
									D_inv_plus_W_B_rm_, L_SigmaI_plus_W_rm_, P_H_sqrt_rm_, L_k_, d_w_plus_I_,
									chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_, true);
							}
							else {
...
#pragma omp parallel for schedule(static)
					...
					vec_t d, d_inv, w_plus_d_inv_sqrt, w_plus_d_inv_sqrt_inv, w_sqrt, L_i, e_i, v_1, v_2;
					// (W + D^(-1))^-0.5
					d_inv = D_inv_rm_.diagonal();
					w_plus_d_inv_sqrt = (information_ll_ + d_inv).cwiseSqrt();
					w_plus_d_inv_sqrt_inv = w_plus_d_inv_sqrt.cwiseInverse();
					...
					// P_L = (L_k * L_k^T + (DW + I)^(-1))
					// with L = ((W + D^(-1))^0.5 * B * W^0.5)^-T and L_k ~ L is an n x k matrix
					int n = d.size();
					w_sqrt = information_ll_.cwiseSqrt();
					...
					if(cg_preconditioner_type_ == "hlfpc" ||
					   cg_preconditioner_type_ == "hlfpc_nystroem_last"){
						...
						// Find the columns of L_k using Pivoted Cholesky factorization
						PivotedCholsekyFactorizationForHLFPC(
							L_k_, B_rm_, w_sqrt, w_plus_d_inv_sqrt_inv, 
							k_, n, err_tol
						);
					}
					else if(cg_preconditioner_type_ == "hlfpc_lanczos"){
						...
						// D^0.5
						den_mat_t D_sqrt = D.cwiseSqrt().asDiagonal(); // to ensure no negative sqrt
					...
					CHECK(rand_vec_trace_I2_.cols() == num_rand_vec_trace_);
					CHECK(rand_vec_trace_I2_.rows() == k_);

					//Get random vectors (z_1, ..., z_t) with Cov(z_i) = P:
					// For P = P_H^0.5 P_L P_H^T/2: z_i = P_H^0.5 r_i, where r_i ~ N(0,P_L)
					// For P_L = (DW + I)^-1 + L_K L_K^T: z_i = P_H^0.5 ((DW + I)^-0.5 r_i + L_K r_j), where r_i, r_j ~ N(0,I)
#pragma omp parallel for schedule(static)   
					for (int i = 0; i < num_rand_vec_trace_; ++i) {
						rand_vec_trace_P_.col(i) = P_H_sqrt_rm_ * (d_w_plus_I_.cwiseInverse().cwiseSqrt().asDiagonal() * rand_vec_trace_I_.col(i) + L_k_ * rand_vec_trace_I2_.col(i));
					}
				}
				else {
...
#pragma omp parallel for schedule(static)   
				...
				CGTridiagVecchiaLaplace(information_ll_, B_rm_, B_t_D_inv_rm_, rand_vec_trace_P_, Tdiags_PI_SigmaI_plus_W, Tsubdiags_PI_SigmaI_plus_W,
					SigmaI_plus_W_inv_Z_, has_NA_or_Inf, num_data, num_rand_vec_trace_, cg_max_num_it_tridiag, cg_delta_conv_, cg_preconditioner_type_,
					D_inv_plus_W_B_rm_, L_SigmaI_plus_W_rm_, P_H_sqrt_rm_, L_k_, d_w_plus_I_, chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_);
				...
				if (!has_NA_or_Inf) {
					...
					if (cg_preconditioner_type_ == "vadu") {
						...
						log_det_Sigma_W_plus_I += D_inv_plus_W_diag.array().log().sum();
							...
							cg_preconditioner_type_ == "hlfpc_lanczos" ||  
							cg_preconditioner_type_ == "hlfpc_rlra"){
						//log|P| = log|P_H| + log|P_L|
						//log|P_H| = log|B^T (D^-1 + W) B| = log|D^-1 + W|
						//log|P_L| = log|L_k L_k^T + (WD + I)^-1| = log|I + L_k^T (WD + I) L_k| + log|(WD + I)^-1|
						vec_t w_plus_d_inv = information_ll_ + D_inv_rm_.diagonal();
						double log_det_P_H = w_plus_d_inv.array().log().sum();
						double log_det_P_L = 2 * ((den_mat_t)chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_.matrixL()).diagonal().array().log().sum() - d_w_plus_I_.array().log().sum();
						log_det_Sigma_W_plus_I += log_det_P_H + log_det_P_L;
					}
					else {
						//log|P| = log|L^T L|
...
#pragma omp parallel for schedule(static)   
	...
	private:
		...
		*/
		...
		/*! \brief High Low Frequency Pivoted Cholesky: L_k the last k columns of L = ((W + D^(-1))^0.5 * B * W^0.5)^-T */
		vec_t d_w_plus_I_;
		/*! \brief High Low Frequency Pivoted Cholesky: k is the nuber of columns of the matrix L_k (see below) */
		int k_;
		/*! \brief High Low Frequency Pivoted Cholesky: L_k the last k columns of L = ((W + D^(-1))^0.5 * B * W^0.5)^-T */
		den_mat_t L_k_;
		/*! \brief High Low Frequency Pivoted Cholesky: P = P_H^0.5 P_L P_H^0.5 with P_H^0.5 =  B^T * D^-0.5 */
		sp_mat_rm_t P_H_sqrt_rm_;
		/*! \brief High Low Frequency Pivoted Cholesky: Factor E of matrix EE^T = (I_k + L_k_^T (D W + I) L_k_)*/
		chol_den_mat_t chol_fact_I_k_plus_L_kt_D_W_plus_I_L_k_vecchia_;
